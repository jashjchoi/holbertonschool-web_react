...
describe('Test CourseListRow.js functions:', () => {
  it("1. CourseListRow renders without crashing", () => {
    const wrapper = shallow(<CourseListRow textFirstCell="test" />);
    expect(wrapper.exists()).toEqual(true);
  });

  it("2. When isHeader = true, verify it renders 1 cell with colspan = 2 when textSecondCell does not exist", () => {
    const wrapper = shallow(<CourseListRow isHeader={true} textFirstCell="test" />);
    expect(wrapper.find('th')).toHaveLength(1);
    expect(wrapper.find('th').props()).toHaveProperty('colSpan', '2');
  });

  it("3. When isHeader = true, verify it renders 2 cells when textSecondCell is present", () => {
  ...
  it("4. When isHeader = false, verify it renders correctly 2 td elements within tr", () => {
